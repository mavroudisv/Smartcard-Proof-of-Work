...
public class MinerClient {
    ...
    boolean runExamples(CardConfig cardCfg) {
        try {
			...
			CardManager cardMngr = new CardManager(false, APPLET_AID);
			...
			cmd = new CommandAPDU(0x00, 0xaa, 0x03, 0xE8, 0x00); //0x03E8->1,000
			resptuple = cardMngr.transmit(cmd);
			cputime = resptuple.time - overhead;
			...
			cmd = new CommandAPDU(0x00, 0xaa, 0x27, 0x10, 0x00); //0x2710->10,000
			resptuple = cardMngr.transmit(cmd);
			cputime = resptuple.time - overhead;
