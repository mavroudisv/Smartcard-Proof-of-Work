...
public class CardManager {
    ...
    private CardChannel connectToCardByTerminalFactory(TerminalFactory factory, int targetReaderIndex) throws CardException {
        ...
        if (card_found) {
            ...
            ResponseTuple responsetuple = transmit(cmd);
            if (responsetuple.response.getSW() == (ISO7816.SW_NO_ERROR & 0xffff)) {
    ...
    public ResponseTuple transmit(CommandAPDU cmd)
		...
		ResponseTuple tuple = new ResponseTuple(response,m_lastTransmitTime);
		//log(tuple);
        return tuple;
    }
	...
	private void log(ResponseTuple resptuple) {
		log(resptuple.response, resptuple.time);
	}
